...
const OrdenCompraAutorizada = ({orden}) => {
    const {id,fecha,pedido01,atencion,cotizacion,pedido,pago,
      ...
      cantidades,detalles,valores,conforme,
      descuento
...
{pedido.map(o => (
        ...
        <div className="grid gap-1 grid-cols-3 py-4 pb-8">
            <div className="px-6 m-auto">
                <p className="border-b border-black text-black font-bold text-sm">
                    <Image width={225} height={80} src="/assets/img/firma.png" alt="Autorizacion-Pendiente" className=""/>
                </p>
                <p className=" text-center">Autorizado : Roberto Hot K.</p>
            </div>
            {pedido.map(o => (
                <div className="px-6 m-auto">
                    <p className="border-b border-black text-black font-bold text-sm">
                    ...
                    <p className=" text-center">Autorizado : {`${o.nombre01}`}</p>
                </div>
            ))}
            {conforme ? (
                pedido.map(o => (
                    <div className="px-6 m-auto" key={o.id}>
                        <p className="border-b border-black text-black font-bold text-sm">
                            <Image width={280} height={80} src={`/assets/img/${o.firma}.png`} alt="Autorizacion-Pendiente" className=""/>
                        </p>
                        <p className="text-center">Recepcionado : {o.nombre01}</p>
                    </div>
                ))
            ) : null}
