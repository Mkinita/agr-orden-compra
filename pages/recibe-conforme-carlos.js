...
import RecepcionConformeCarlos from '../components/RecepcionConformeCarlos'
...
export default function Admin() {
    ...
    const fetcher = () => axios('/api/recibe-conforme-carlos').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/recibe-conforme-carlos',fetcher,{refreshInterval: 100} )

    const [ datos, setDatos ] = useState([])
    const [ buscar, setBuscar ] = useState("")
  
    //función para traer los datos de la API
    const URL = '/api/recibe-conforme-carlos'
