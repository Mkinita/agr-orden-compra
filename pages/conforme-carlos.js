...
import ListaConforme from '../components/ListaConforme'
...
export default function Admin() {
    ...
    const fetcher = () => axios('/api/conforme-carlos').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/conforme-carlos',fetcher,{refreshInterval: 100} )

    const [ datos, setDatos ] = useState([])
    const [ buscar, setBuscar ] = useState("")
  
    //función para traer los datos de la API
    const URL = '/api/conforme-carlos'
            ...
            <table className="table-auto w-full text-center bg-white text-gray-700">
                ...
                <tbody>
                    <tr className="bg-white border-b text-sm font-bold">
                        <td className="px-6 py-2 w-1/4 text-center border border-amber-400">Fecha</td>
                        <td className="px-6 py-2 w-1/4 text-center border border-amber-400">Solicitud</td>
                        <td className="px-6 py-2 w-1/4 text-center border border-amber-400">OC</td>
