...
export default function Admin() {
    ...
    const fetcher = () => axios('/api/solicitud-nueva-carlos').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/solicitud-nueva-carlos',fetcher,{refreshInterval: 100} )

    const [ users, setUsers ] = useState([])
    const [ search, setSearch ] = useState("")

    //función para traer los datos de la API
    const URL = '/api/solicitud-nueva-carlos'
