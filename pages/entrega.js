...
export default function Admin() {
    ...
    const fetcher = () => axios('/api/ordenes-porfirmar-gavino-conforme').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/ordenes-porfirmar-gavino-conforme',fetcher,{refreshInterval: 100} )

    const [ datos, setDatos ] = useState([])
    const [ buscar, setBuscar ] = useState("")
  
    //función para traer los datos de la API
    const URL = '/api/ordenes-porfirmar-gavino-conforme'
